...
...where ITEM and PRICE are replaced with the featured item name and price. You can do this by _either_ (1) fetching the featured item from `https://cs571.org/api/s24/hw3/featured-sale-item` or (2) finding the featured item in the `saleItems` array. If you decide to find the featured item in the `saleItems` array, you may _not_ assume the last item is the featured item; you must check for the `featured` prop.
...
Using your knowledge of React and visual design, improve your Badger Mart interface based on 3 design elements _or_ principles. **This means changing your React code.** Following, please describe these changes by modifying this `README.md` below. **Clearly indicate which 3 design elements or principles you used.**

1. Emphasis: Changed the text of featured item name into bold text to give the emphasis on the featured item and establish the visual hierarchy
2. Negative Space: Used a negative space to add the space to the welcome message so that the user clearly sees the welcome message
3. Consistency: setted box-sizing to border-box and padding around the box to the all items for consistency
...
![Visual Design by Hye Won](_figures/visualDesign_hyewon.png)
